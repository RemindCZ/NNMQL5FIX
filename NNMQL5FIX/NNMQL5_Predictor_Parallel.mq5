...
void SaveModelBin()
{
   if(g_h <= 0) { Print("Warning: Network not valid."); return; }
...
{
   int total = Bars(_Symbol, _Period);
   if(total < TrainBars + Lookback + 5) return;
...
bool BuildDataset(const double &o[], const double &h[], const double &l[], const double &c[], int total)
{
   ...
   if(start < Lookback + 2) return false;
   ...
   if(g_batch_cursor >= g_N) g_batch_cursor = 0;
...
bool PrepareMiniBatch(int batch_sz)
{
   ...
   for(int r=0; r<bs; r++)
   {
      ...
      int dst = r * Lookback;
      for(int k=0;k<Lookback;k++) g_bX[dst+k] = g_X[src+k];
...
bool TrainOneTick(double &mse_out)
{
   ...
   {
      if(!PrepareMiniBatch(bs)) return false;
...
{
   if(!g_ready || g_h == 0) return;

   int start = total - MathMax(RealtimeBars, Lookback + 5);
   if(start < Lookback + 2) start = Lookback + 2;
   ...
   for(int i = start; i < total; i++)
   {
      ...
      if(UseLogReturns) pred_price = curr_price * MathExp(real_val);
      else             pred_price = real_val;

      PredBuffer[i] = pred_price;

      // Target +1 (realtime)
      if(i + 1 < total) TgtBuffer[i] = GetVal(i + 1, o,h,l,c);
      else             TgtBuffer[i] = EMPTY_VALUE;
   }
}
...
void DrawFuture(const double &o[], const double &h[], const double &l[], const double &c[], int total, const datetime &tm[])
{
   if(!ShowFuture || !g_ready || g_h==0) return;
